...
import top.anagke.kio.util.TempFiles.alloc
import top.anagke.kio.util.TempFiles.allocLocal
import top.anagke.kio.util.TempFiles.allocSystem
import top.anagke.kio.util.TempFiles.free
import top.anagke.kio.util.TempFiles.register
 ...
 * cleaned immediately (if it already exists). Then, call [alloc] with
 * the registered temp directory to obtain a temp file. Finally call
 * [free] to free the allocated temp file.
 *
 ...
 *
 * Don't worry if there are some temp files which haven't been
 * freed. There's a shutdown hook, freeing all allocated temp files
 * and cleaning all registered temp directories. In the worst case,
 * the JVM may halt without calling the shutdown hook. However,
 * [register] cleans the temp directory while registering. So register
 * the temp directory on program startups is the best practice.
 */
object TempFiles {
    ...
    /**
     * The default temp directory specified by the OS. It is
     * based on the JVM property `java.io.tmpdir`, resolving
     * `top.anagke.kio_TempFiles` directory.
     *
     * In Unix-like systems, the base temp directory is usually `/tmp`
     * or `/var/tmp`. In windows, the base temp directory is usually
     * `%tmp%`. However, this property could be changed by adding JVM
     * option `-Djava.io.tmpdir=<path>`
     */
    val SYSTEM_TEMP_DIR: Path = Paths.get(System.getProperty("java.io.tmpdir"), "top.anagke.kio_TempFiles")

    /**
     * The default temp directory which is OS independent. It is based
     * on the cwd resolving `top.anagke.kio_TempFiles`.
     */
     ...
     *
     * **WARNING**: This method cleans all files and directories in the
     * temp directory. Make sure there are no important files in the
     * temp directory. The temp directory will also be cleans before
     * the JVM shutdowns.
     */
    ...
    /**
     * Allocates a temp file inside the given temp directory. To ensure
     * all temp files are cleaned after the JVM shutdowns, the temp
     * directory must be registered via [register].
     *
    ...
    /**
     * Frees the given temp file. To ensure not to delete any important
     * file, the given temp file must be allocated via [alloc].
     *
     ...
     * [TempFileNotFoundException]. Freeing a same temp file multiple
     * times throws a [TempFileNotFoundException] too.
     *
     * @param tempFile the given temp file, should be allocated via
     *     [alloc]
     */
    ...
    private fun newTempFileName(): String = UUID.randomUUID().toString()
     ...
     */
    fun allocSystem() = alloc(SYSTEM_TEMP_DIR)
     ...
     * More details in [alloc].
     */
    fun allocLocal() = alloc(LOCAL_TEMP_DIR)
...
/**
 * Executes the given block with a new allocated temp file and then free
 * it correctly whether an exception is thrown or not.
 *
 * @param tempDir the temp directory
 * @param block the given block
 */
fun TempFiles.useTempFile(tempDir: Path, block: (Path) -> Unit) {
    val tempFile = alloc(tempDir)
    try {
        block(tempFile)
    } finally {
        free(tempFile)
